...
export default function JuniorsPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "users"), snap => {
      ...
      // Filter & Sort
      const filtered = list.filter(m => classifyMember(m) === "juniors").sort((a, b) => b.xp - a.xp);
      setMembers(filtered);
      setLoading(false);
    });
    return () => unsub();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <Loader2 className="w-12 h-12 text-[#2d5da1]/40 animate-spin" />
        <p className="font-patrick font-bold text-[#2d5da1]/50 uppercase tracking-widest text-sm animate-pulse">Gathering Juniors...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Header */}
      <div className="hand-card bg-[#e8f4f8] border-[3px] border-[#2d5da1] shadow-[6px_6px_0_0_#2d2d2d] p-6 md:p-10 flex flex-col sm:flex-row items-center sm:justify-between rounded-[var(--radius-wobbly)] gap-6 text-center sm:text-left">
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="w-20 h-20 bg-[#2d5da1] border-[3px] border-[#2d2d2d] rounded-[var(--radius-wobbly)] flex items-center justify-center text-white shadow-[4px_4px_0_0_#2d2d2d] rotate-3">
            <BookOpen size={40} strokeWidth={2.5} />
          </div>
          <div>
            <h1 className="font-kalam font-bold text-4xl md:text-5xl text-[#2d5da1] mb-2">Juniors</h1>
            <p className="font-patrick text-[#2d2d2d]/60 text-lg">1st, 2nd & 3rd Year</p>
          </div>
        </div>
        <div className="font-kalam font-bold text-5xl text-[#2d5da1]">
          {members.length}
        </div>
      </div>
      ...
      {paginated.length > 0 ? (
        ...
        <div className="py-24 border-[3px] border-dashed border-[#2d2d2d]/20 rounded-[var(--radius-wobbly)] text-center bg-white/30">
          <p className="font-patrick text-[#2d2d2d]/40 italic text-xl">
            {search ? "No members match your search." : "No juniors found."}
          </p>
        </div>
      )}
